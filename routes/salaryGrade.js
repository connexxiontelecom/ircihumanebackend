...
router.post('/add-salary-grade', auth(), async function (req, res, next) {
  try {
    const schema = Joi.object({
      ...
      sg_col_allowance: Joi.number().precision(2).required()
    });
    ...
    if (salaryGradeRequest.sg_minimum < salaryGradeRequest.sg_midpoint && salaryGradeRequest.sg_midpoint < salaryGradeRequest.sg_maximum) {
      const existingSalaryGrade = await salaryGrade.findSalaryGradeByName(salaryGradeRequest.sg_name);
      if (!_.isEmpty(existingSalaryGrade)) {
        return res.status(400).json('Salary Grade Already Exists');
      }
      await salaryGrade.addSalaryGrade(salaryGradeRequest);
      await logs.addLog({
