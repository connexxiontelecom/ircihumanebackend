...
router.post('/add-time-allocation', auth(), async function (req, res, next) {
  try {
    ...
    const empTimesheet = await timeSheet.findTimeSheetMonthEmployee(
      parseInt(req.body.ta_emp_id),
      parseInt(req.body.ta_month),
      parseInt(req.body.ta_year)
    );
      ...
      if (_.isNull(recordExist) || _.isEmpty(recordExist)) {
        await authorizationAction
          .registerTimeAllocationAction(
            2,
            data.ta_ref_no,
            employeeData.emp_supervisor_id,
            0,
            'Time allocation/time sheet initialized.',
            req.body.ta_month,
            req.body.ta_year
          )
          .then(async (val) => {
            ...
            const notifyEmployee = await handleInAppEmailNotifications(
              employeeData.emp_first_name,
              subject,
              body,
              url,
              employeeData.emp_office_email,
              employeeData.emp_id
            );
            ...
            if (!_.isNull(supervisor) || !_.isEmpty(supervisor)) {
              const notifySupervisor = await handleInAppEmailNotifications(
                supervisor.emp_first_name,
                "There's a new timesheet for you to assess",
                "There's a new timesheet for you to assess",
                url,
                supervisor.emp_office_email,
                supervisor.emp_id
              );
            }
...
router.patch('/re-assign-timesheet/:ref_no', auth(), async function (req, res) {
  try {
    ...
    await handleInAppEmailNotifications(
      reAssignedOfficer.emp_first_name,
      'Time sheet re-assignment',
      `Timesheet re-assigned to ${reAssignedOfficer.emp_first_name} ${reAssignedOfficer.emp_last_name}`,
      'time-sheet-authorization',
      reAssignedOfficer.emp_office_email,
      reAssignedOfficer.emp_id
    );
    await handleInAppEmailNotifications(
      employeeData.emp_first_name,
      'Your time sheet was re-assigned',
      `Your time sheet was now assigned to ${reAssignedOfficer.emp_first_name} ${reAssignedOfficer.emp_last_name}`,
      'timesheets',
      employeeData.emp_office_email,
      employeeData.emp_id
    );
...
router.get('/salary-mapping-master/:month/:year/:locationId', auth(), async function (req, res, next) {
  try {
    ...
    const salaryMappingMasterData = await salaryMappingMasterService
      .getSalaryMappingMasterByMonthYearLocation(month, year, locationId)
      .then((data) => {
        return data;
      });
...
router.post('/timesheet-application-tracking-report', auth(), async function (req, res) {
  try {
    ...
    const salaryEmployees =
      location === 0
        ? await salaryModel.generateAllEmployeesTimesheetReport(empIds, month, year)
        : await salaryModel.generateEmployeesTimesheetReportByLocation(empIds, location, month, year);
