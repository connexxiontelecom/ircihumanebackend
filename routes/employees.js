...
router.patch('/update-employee-nhf/:emp_id', auth(), async function (req, res, next) {
  try {
    ...
    const schema = Joi.object({
      nhf: Joi.boolean().required()
    });
    const validationResult = schema.validate(employeeData);
    if (validationResult.error) {
      return res.status(400).json(validationResult.error.details[0].message);
    }
    await employees.setNhf(empId, employeeData.nhf);
    await logs.addLog({
      log_user_id: req.user.username.user_id,
      log_description: 'Updated Employee Details',
      log_date: new Date()
...
router.patch('/upload-profile-pic/:empId', auth(), async function (req, res, next) {
  try {
    ...
    const uploadResponse = await uploadFile(req.files.profilepic)
      .then((response) => {
        return response;
      })
      .catch((err) => {
...
router.post('/upload-documents/:empId', auth(), async function (req, res, next) {
  try {
    ...
    if (Array.isArray(docs)) {
      ...
      const uploadResponse = await uploadFile(docs)
        .then((response) => {
          return response;
        })
        .catch((err) => {
...
router.post('/upload-files', auth(), async function (req, res, next) {
  try {
    ...
    let uploadResponse = await uploadFile(req.files.test)
      .then((response) => {
        return response;
      })
      .catch((err) => {
...
router.post('/get-employee-report', auth(), async function (req, res, next) {
  try {
    ...
    return res.status(200).json(`An error occurred while fetching Employee ` + err);
    next(err);
  }
});
...
router.post('/upload-contract-end-date', auth(), async function (req, res, next) {
  try {
    ...
    if (fileExt === '.csv' || fileExt === '.xlsx' || fileExt === '.xls') {
      let uploadResponse = await uploadFile(req.files.contract_end)
        .then((response) => {
          return response;
        })
        .catch((err) => {
