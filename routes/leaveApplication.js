      ...
      if (number === 1) {
        ...
        if (three > 0) {
          await addToLeaveAccrual(
            leave.leapp_empid,
            threeDate.getFullYear(),
            threeDate.getMonth() + 1,
        ...
        if (seven > 0) {
          await addToLeaveAccrual(
            leave.leapp_empid,
            sevenDate.getFullYear(),
            sevenDate.getMonth() + 1,
            leave.leapp_leave_type,
            seven,
            leave.leapp_id
          );
        ...
        if (eight > 0) {
          await addToLeaveAccrual(
            leave.leapp_empid,
            eightDate.getFullYear(),
            eightDate.getMonth() + 1,
            leave.leapp_leave_type,
            eight,
            leave.leapp_id
          );
        ...
        if (eleven > 0) {
          await addToLeaveAccrual(
            leave.leapp_empid,
            elevenDate.getFullYear(),
            elevenDate.getMonth() + 1,
            leave.leapp_leave_type,
            eleven,
            leave.leapp_id
          );
        ...
        if (twelve > 0) {
          await addToLeaveAccrual(
            leave.leapp_empid,
            twelveDate.getFullYear(),
            twelveDate.getMonth() + 1,
            leave.leapp_leave_type,
            twelve,
            leave.leapp_id
          );
...
router.patch('/re-assign-leave/:leaveId', auth(), async function (req, res) {
  try {
    ...
    const assignedNotify = await handleInAppEmailNotifications(
      ...
      body,
      url,
      assignedOfficer.emp_office_email,
      assignedOfficer.emp_id
    );
...
router.post('/leave-planner', async function (req, res) {
  try {
    ...
    const empId = parseInt(req.body.emp_id);
    const fyYear = req.body.fy_year;
    const annualLeaveId = 1;
    const sickLeaveId = 2;
    ...
    for (const supervisedEmployee of supervisedEmployees) {
      const { sa_emp_id } = supervisedEmployee;
      const emp = await employee.getEmployee(sa_emp_id);
      const leavePlannerObj = {
        t7: emp.emp_unique_id,
        t6: emp.location?.l_t6_code,
        first_name: emp?.emp_first_name,
        last_name: emp?.emp_last_name,
