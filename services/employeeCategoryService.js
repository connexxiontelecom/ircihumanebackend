...
const employeeCategory = require("../models/EmployeeCategories")(
  sequelize,
  Sequelize.DataTypes
);
...
const getEmployeeCategories = async (req, res) => {
  const employeeCats = await employeeCategory.findAll({
    attributes: ["ec_name", "ec_id", "ec_type"],
  });
...
const setNewEmployeeCategory = async (req, res, next) => {
  try {
    const schema = Joi.object({
      ...
      employee_cat_type: Joi.string().required(),
    });
    ...
    await employeeCategory
      .create({
        ec_name: req.body.employee_cat_name,
        ec_type: req.body.employee_cat_type,
      })
      .catch(errHandler);
    //Log
    const logData = {
    ...
    logs.addLog(logData).then((logRes) => {
      return res
        .status(200)
        .json(
          `New employee category :  ${req.body.employee_cat_name} was successfully saved`
        );
    });
  } catch (e) {
  ...
  try {
    ...
    logs.addLog(logData).then((logRes) => {
      return res
        .status(200)
        .json(
          `Your changes on   ${req.body.employee_cat_name} were saved successfully.`
        );
    });
  } catch (e) {
...
const getEmployeeCategoryByType = async (req, res) => {
  const ec_type = req.params.type;
  const ec = await employeeCategory.findAll({ where: { ec_type } });
  return res.status(200).json(ec);
};
